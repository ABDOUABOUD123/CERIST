...
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticatedOrReadOnly])  # Allows GET without auth
def latest_volumes(request):
    volumes = Article.objects.exclude(volume__isnull=True)\
                .values('volume')\
                .annotate(article_count=Count('id'), latest_date=Max('publication_date'))\
